...
func AddArtistRoutes(router *httprouter.Router, app *infra.Deps, rateLimiter *middleware.RateLimiter) {
	...
	router.HandlerFunc(http.MethodPost, "/api/v1/artists/:id/members", rateLimiter.Limit(authmidware(artists.AddArtistMember(app))))

	router.HandlerFunc(http.MethodPut, "/api/v1/artists/:id/members/:memberId", rateLimiter.Limit(authmidware(artists.UpdateArtistMember(app))))

	router.HandlerFunc(http.MethodDelete, "/api/v1/artists/:id/members/:memberId", rateLimiter.Limit(authmidware(artists.DeleteArtistMember(app))))
...
func AddMusicRoutes(router *httprouter.Router, app *infra.Deps, rateLimiter *middleware.RateLimiter) {
	...
	// --------------------------- PLAYLISTS ---------------------------
	router.HandlerFunc(http.MethodGet, "/api/v1/musicon/user/playlists", rateLimiter.Limit(authmidware(musicon.GetUserPlaylists(app))))

	router.HandlerFunc(http.MethodGet, "/api/v1/musicon/user/liked", rateLimiter.Limit(authmidware(musicon.GetUserLikes(app))))
	...
	// Playlist details
	router.HandlerFunc(http.MethodGet, "/api/v1/musicon/playlists/:playlistid/songs", rateLimiter.Limit(authmidware(musicon.GetPlaylistSongs(app))))
	...
	router.HandlerFunc(http.MethodPost, "/api/v1/musicon/user/liked/:songid", rateLimiter.Limit(authmidware(musicon.LikeSong(app))))

	// Unlike song (idempotent)
	router.HandlerFunc(http.MethodDelete, "/api/v1/musicon/user/liked/:songid", rateLimiter.Limit(authmidware(musicon.UnlikeSong(app))))
