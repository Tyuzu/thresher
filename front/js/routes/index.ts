import {
    createheader
} from "../components/layout/header.ts";
import {
    createNav,
    highlightActiveNav
...
import {
    Footer
} from "../components/layout/footer.ts";
import {
    track
} from "../services/activity/metrics.ts";
import {
    startPerfMonitoring
} from "../services/activity/perfMonitor.ts";
import {
 ...
 */
function toBrowserTarget(target, mode = "history") {
...
/* =========================================================
   AUTH HYDRATION
========================================================= */
export function hydrateAuthState(force = false) {
    if (layoutState.isHydrated && !force) {
        return;
    }
    const state = getState() || {};
...
function refreshStaticLayout() {
    ...
    if (header) {
        ...
        if (updatedHeader) {
            header.replaceChildren(updatedHeader);
        }
    }
    if (nav) {
...
/* =========================================================
   CONTENT LOADING
========================================================= */
export async function loadContent(rawLocation = getCurrentAppLocation(), {
    redirectDepth = 0
} = {}) {
    const {
        main
...
export async function navigate(path, {
    ...
    try {
         ...
         */
        if (storeRedirect && currentParsed.path !== "/" && currentParsed.path !== "/login" && currentParsed.path !== "/logout") {
         ...
         * hash routing.
         */
        const browserTarget = target.startsWith("#/") ? target : target.startsWith("/") ? target : `/${target}`;
        if (replace) {
            history.replaceState(null, "", browserTarget);
        } else {
...
subscribe("token",
    (token) => {
        ...
        const target = redirect && redirect.startsWith("/") && redirect !== "/login" && redirect !== "/logout" ? redirect : "/";
        queueMicrotask(() => {
            navigate(target, {
                replace: true
            }).catch((error) => {
