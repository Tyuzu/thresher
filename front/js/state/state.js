...
function writeStorage(key, value) {
  try {
    ...
    console.warn(
      `Failed persisting state key "${key}":`,
      error
    );
  }
}
...
const initialRolesRaw =
  safeParse("roles");

const initialPermissionsRaw =
  safeParse(
    "permissions"
  );
...
function setState(
  keyOrObject,
  persistOrValue = false,
  maybeValue = undefined
) {
  let persist = false;

  const updateSingleKey = (
    key,
    value
  ) => {
    if (
...
function subscribe(
  ...
  if (
    ...
    throw new Error(
      `Cannot subscribe to invalid key: ${key}`
    );
  }
...
function unsubscribeDeep(
  ...
  if (
    deepListeners.get(path)
      ?.size === 0
  ) {
    deepListeners.delete(
      path
    );
  }
}
...
function setRouteModule(
  ...
  state.routeCache.set(
    path,
    moduleData
  );
}

function hasRouteModule(
  path
) {
  return state.routeCache.has(
    path
  );
}
